...
async function main(deployTokens: boolean) {
  ...
  if(deployTokens) {
    ...
    const bond = await Bond.deploy(TestParams.Treasury, TestParams.USDT, TestParams.BitCouney, TestParams.Treasury );
  
    await bond.deployed();
  
    console.log(`Bond contract deployed to ${bond.address}`);
  }
